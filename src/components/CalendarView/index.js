          ...
          {allDaysInMonth().map((day, idx) => {
            return (
              <div key={idx} className={colStartClasses[moment(day).day().toString()] + " border border-solid w-full h-28  "}>
                <p className={`inline-block flex items-center  justify-center h-8 w-8 rounded-full mx-1 mt-1 text-sm cursor-pointer hover:bg-base-300 ${isToday(day) && " bg-blue-100 light:bg-blue-400 light:hover:bg-base-300 light:text-white"} ${isDifferentMonth(day) && " text-slate-400 light:text-slate-600"}`} onClick={() => addNewEvent(day)}> { moment(day).format("D") }</p>
                {
