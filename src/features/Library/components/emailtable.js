...
function UserChannels() {
    ...
    const {library:{emailTable}}=jsondata
    const startIndex = (currentPage - 1) * itemsPerPage;
    const visibleData = emailTable.slice(startIndex, startIndex + itemsPerPage);
  
    const totalPages = Math.ceil(emailTable.length / itemsPerPage);
    ...
    return (
      <TitleCard title={"Content library-email"} className="rounded-t-none" topMargin="mt-2" TopSideButtons={<TopSideButtons applySearch={applySearch} applyFilter={applyFilter} removeFilter={removeFilter}/>}>
